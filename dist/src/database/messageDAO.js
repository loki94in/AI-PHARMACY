...
export function deleteTemplate(locale, key) {
    db.prepare('DELETE FROM message_templates WHERE locale = ? AND key = ?').run(locale, key);
}
export function listTemplates(locale) {
