...
export const salesInvoiceMap = new Map(); // legacy order_id → new sales_invoices.id
...
async function ensureInvoiceNoUnique(invoiceNo, legacyId, db) {
...
// ─── Orders → sales_invoices ────────────────────────────────
let salesBatch = [];
export async function importOrder(row, db) {
    ...
    if (status && status !== 'BILL' && status !== 'DELIVERED' && status !== 'COMPLETED')
        return;
...
export async function flushSalesInvoices(db) {
    if (salesBatch.length === 0)
        return;
    await db.run('BEGIN TRANSACTION');
    for (const s of salesBatch) {
        const result = await db.run(`INSERT INTO sales_invoices (invoice_no, customer_id, date, total_amount, tax_amount, doctor_id, payment_medium, roff, cgst_value, sgst_value, igst_value, legacy_id, business_date)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [s.invoice_no, s.customer_id, s.date, s.total_amount, s.tax_amount, s.doctor_id, s.payment_medium, s.roff, s.cgst_value, s.sgst_value, s.igst_value, s.legacy_id, s.business_date]);
...
let saleItemBatch = [];
export async function importOrderItem(row, db) {
    ...
    if (!invoiceId)
        return; // Parent order was deleted or not imported
    // Resolve medicine
...
export async function flushSaleItems(db) {
    if (saleItemBatch.length === 0)
        return;
    await db.run('BEGIN TRANSACTION');
    for (const si of saleItemBatch) {
        await db.run(`INSERT INTO sale_items (invoice_id, inventory_id, quantity, unit_price, mrp, batch_no, cgst_value, sgst_value, discount_per, legacy_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [si.invoice_id, si.inventory_id, si.quantity, si.unit_price, si.mrp, si.batch_no, si.cgst_value, si.sgst_value, si.discount_per, si.legacy_id]);
    }
    await db.run('COMMIT');
    saleItemBatch = [];
}
