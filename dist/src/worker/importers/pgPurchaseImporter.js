...
export const batchMap = new Map(); // legacy batch_id → new inventory_master.id
export const purchaseMap = new Map(); // legacy inventory_id → new purchases.id
...
export async function importBatch(row, db) {
    const legacyBatchId = row['batch_id'];
    const legacyMedicineId = row['medicine_id'];
    const deleted = row['deleted'];
    if (!legacyBatchId || !legacyMedicineId || deleted === 't')
        return;
    // Resolve medicine
    const medicineId = medicineMap.get(legacyMedicineId);
    if (!medicineId)
        return; // Medicine was deleted or not imported
    batchBatch.push({
...
export async function flushBatches(db) {
    if (batchBatch.length === 0)
        return;
    await db.run('BEGIN TRANSACTION');
    for (const b of batchBatch) {
        const result = await db.run(`INSERT INTO inventory_master (medicine_id, batch_no, expiry_date, rack_location, cost_price, unit_price, mrp, legacy_batch_id, quantity)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [b.medicine_id, b.batch_no, b.expiry_date, b.rack_location, b.cost_price, b.unit_price, b.mrp, b.legacy_batch_id, b.quantity]);
...
// ─── Inventory → purchases (purchase bill headers) ──────────
let purchaseBatch = [];
export async function importInventory(row, db) {
    ...
    if (row['is_transfer'] === 't' || row['is_transfer'] === 'true')
        return;
    // Resolve distributor
...
export async function flushPurchases(db) {
    if (purchaseBatch.length === 0)
        return;
    await db.run('BEGIN TRANSACTION');
    for (const p of purchaseBatch) {
        const result = await db.run(`INSERT INTO purchases (distributor_id, invoice_no, date, total_amount, cgst_value, sgst_value, igst_value, roff, status, legacy_id, business_date)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [p.distributor_id, p.invoice_no, p.date, p.total_amount, p.cgst_value, p.sgst_value, p.igst_value, p.roff, p.status, p.legacy_id, p.business_date]);
...
let purchaseItemBatch = [];
export async function importInventoryMedicine(row, db) {
    ...
    if (!purchaseId)
        return; // Parent purchase was deleted or not imported
    // Resolve medicine
...
export async function flushPurchaseItems(db) {
    if (purchaseItemBatch.length === 0)
        return;
    await db.run('BEGIN TRANSACTION');
    for (const pi of purchaseItemBatch) {
        await db.run(`INSERT INTO purchase_items (purchase_id, medicine_id, batch_no, quantity, free_qty, cost_price, mrp, hsn_code,
        cgst_per, cgst_value, sgst_per, sgst_value, igst_per, igst_value, scheme_per, scheme_value, cd_value, legacy_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [pi.purchase_id, pi.medicine_id, pi.batch_no, pi.quantity, pi.free_qty, pi.cost_price, pi.mrp, pi.hsn_code,
