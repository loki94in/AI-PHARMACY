 ...
 */
export async function processReturnsLine(sqlLine, db) {
    ...
    try {
        ...
        if (typeRaw === 'sale') {
            ...
            const saleResult = await db.get('SELECT id FROM sales_invoices WHERE invoice_no = ?', [originalInvoiceNumber]);
            originalInvoiceId = saleResult ? saleResult.id : null;
        }
        else if (typeRaw === 'purchase') {
            // Look up in purchases table
            const purchaseResult = await db.get('SELECT id FROM purchases WHERE invoice_no = ?', [originalInvoiceNumber]);
        ...
        await db.run('INSERT INTO returns (return_no, original_invoice_id, type, date, total_amount) VALUES (?, ?, ?, ?, ?)', [returnNo, originalInvoiceId, typeRaw, normalizedDate, totalAmount]);
        return true;
    }
    catch (error) {
