...
export async function processInventoryLine(sqlLine, db) {
    ...
    try {
        ...
        await db.run('INSERT INTO inventory_master (medicine_id, quantity, rack_location, batch_no, expiry_date) VALUES (?, ?, ?, ?, ?)', [medicineId, quantity, rackLocation, batchNo, normalizedExpiryDate]);
        return true;
    }
    catch (error) {
