...
function findApiReference(text) {
...
export async function processJob(job) {
    ...
    try {
        ...
        for (const n of names) {
            await db.run(`INSERT INTO medicines (name, api_reference)
         SELECT ?, ?
         WHERE NOT EXISTS (SELECT 1 FROM medicines WHERE lower(name)=lower(?))`, n, apiRef, n);
        }
