...
// Importers
import { clearAllMaps, distributorMap, doctorMap, patientMap, medicineMap, importCategory, importManufacturer, importDistributor, flushDistributors, importDoctor, flushDoctors, importPatient, flushPatients, importMedicine, flushMedicines, } from './importers/pgMasterImporter.js';
import { batchMap, purchaseMap, clearPurchaseMap, importBatch, flushBatches, importInventory, flushPurchases, importInventoryMedicine, flushPurchaseItems, } from './importers/pgPurchaseImporter.js';
import { salesInvoiceMap, clearSalesMap, importOrder, flushSalesInvoices, importOrderItem, flushSaleItems, } from './importers/pgSalesImporter.js';
import { returnMap, clearReturnsMap, importReturnOrder, flushReturns, importReturnOrderItem, flushReturnItems, importStockEffect, flushStockLedger, } from './importers/pgReturnsImporter.js';
 ...
 */
async function parseAndImportPgDump(sqlPath) {
 ...
 */
async function streamPgDump(sqlPath, handlers) {
    const fileStream = fs.createReadStream(sqlPath, { encoding: 'utf8' });
    const rl = readline.createInterface({ input: fileStream, crlfDelay: Infinity });
 ...
 */
async function generateMigrationReport(db, stats) {
    ...
    fs.writeFileSync(path.join(reportsDir, 'migration_summary.json'), JSON.stringify(summary, null, 2));
    ...
    fs.writeFileSync(path.join(reportsDir, 'row_counts.json'), JSON.stringify(counts, null, 2));
    console.log('Migration reports saved to:', reportsDir);
}
/**
 * Legacy SQL parser (INSERT-based) — kept for backward compatibility.
 */
async function parseAndImportLegacySQL(sqlPath) {
