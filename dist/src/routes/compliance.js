...
router.post('/add', async (req, res) => {
    ...
    try {
        const db = await open({ filename: DB_PATH, driver: sqlite3.Database });
        await db.run('INSERT INTO action_logs (action_type, description) VALUES (?, ?)', ['COMPLIANCE_ENTRY', `Date: ${date} | Product: ${product} | Patient: ${patient_id} | Doctor: ${doctor_id} | Lic: ${license_no} | Qty: ${qty} | Bill: ${bill_no}`]);
        await db.close();
...
router.post('/add-schedule-h1', async (req, res) => {
    ...
    try {
        ...
        await db.run('INSERT INTO action_logs (action_type, description) VALUES (?, ?)', ['SCHEDULE_H1_DISPENSE', `Drug: ${drug_name} | Patient: ${patient_name} | Doctor: ${doctor_name} | Schedule: H1`]);
        await db.close();
