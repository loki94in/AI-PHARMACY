...
router.post('/cart/add', async (req, res) => {
    try {
        ...
        const success = await telegramPrescriptionService.addItemToCart(parsedChatId, medicineName.trim(), parsedQuantity);
        if (success) {
...
router.post('/bill/generate', async (req, res) => {
    ...
    try {
        ...
        for (const item of items) {
            await db.run('INSERT INTO sale_items (invoice_id, inventory_id, quantity, unit_price) VALUES (?, ?, ?, ?)', [invoiceId, item.inventory_id, item.quantity, item.unit_price]);
            // Decrement stock
