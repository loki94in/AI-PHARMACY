...
router.get('/audit/queue', async (req, res) => {
    try {
        ...
        const pending = queue.filter((item) => item.status === 'pending_human_review');
        res.json(pending);
    }
    catch (err) {
...
router.post('/audit/resolve', async (req, res) => {
    ...
    try {
        ...
        if (action === 'add_to_db') {
            ...
            let medicineId;
            if (!med) {
                const result = await db.run(`INSERT INTO medicines (name, mrp) VALUES (?, ?)`, [name.trim(), mrp || 0]);
            ...
            if (batchNumber && batchNumber.trim() !== '') {
                await db.run(`INSERT INTO inventory_master (medicine_id, batch_no, expiry_date, quantity) VALUES (?, ?, ?, ?)`, [medicineId, batchNumber.trim(), expiryDate || null, 0]);
            }
            await db.close();
        }
