...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        await db.run('INSERT INTO action_logs (action_type, description) VALUES (?, ?)', ['LEARNING_DATA', JSON.stringify(payload).slice(0, 200)]);
        await db.close();
...
router.post('/analyze', async (req, res) => {
    ...
    try {
        ...
        Object.keys(patterns).forEach(field => {
            ...
            const match = headers.find(header => possibleMatches.some(pattern => header.toLowerCase().includes(pattern.toLowerCase())));
            if (match) {
                mapping[field] = match;
            }
        });
        ...
        if (headers.length >= 5) {
            ...
            if (!mapping.price)
                mapping.price = headers[2];
            if (!mapping.expiry_date)
                mapping.expiry_date = headers[3];
            if (!mapping.batch_number)
                mapping.batch_number = headers[4];
        }
...
router.post('/apply-model', async (req, res) => {
    ...
    if (!rawData || !mapping)
        return res.status(400).json({ error: 'rawData and mapping required' });
    try {
        const db = await open({ filename: DB_PATH, driver: sqlite3.Database });
        // For demo, store raw data and mapping in action_logs
        await db.run('INSERT INTO action_logs (action_type, description) VALUES (?, ?)', ['LEARNING_APPLY', JSON.stringify({ rawData, mapping })]);
        await db.close();
