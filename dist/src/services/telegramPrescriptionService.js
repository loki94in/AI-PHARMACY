...
class TelegramPrescriptionService {
     ...
     */
    async handlePrescriptionResult(chatId, result, caption = '', bot) {
        try {
            ...
            const existingItemIndex = cart.items.findIndex(item => item.inventory_id === inventoryDetails.id);
     ...
     */
    async findMedicineIdByName(name) {
        try {
            const db = await open({ filename: DB_PATH, driver: sqlite3.Database });
            // First try exact match
            const exactMatch = await db.get('SELECT id FROM medicines WHERE LOWER(name) = LOWER(?)', [name]);
            if (exactMatch) {
                await db.close();
            ...
            if (filterResult.matches.length > 0) {
                ...
                const medicine = await db.get('SELECT id FROM medicines WHERE LOWER(name) = LOWER(?)', [bestMatch]);
                await db.close();
     ...
     */
    async getInventoryDetails(medicineId) {
        try {
            const db = await open({ filename: DB_PATH, driver: sqlite3.Database });
            const inventory = await db.get(`SELECT im.id, im.quantity, im.mrp, im.batch_number, im.expiry_date
         FROM inventory_master im
         WHERE im.medicine_id = ? AND im.quantity > 0
         ORDER BY im.quantity DESC LIMIT 1`, [medicineId]);
            await db.close();
            return inventory;
        }
        catch (error) {
     ...
     */
    async addItemToCart(chatId, medicineName, quantity = 1) {
        try {
            const cart = this.getOrCreateCart(chatId);
            // Find medicine
    ...
    async sendMedicineNotFoundMessage(chatId, medicineName, bot) {
    ...
    async sendNoInventoryMessage(chatId, medicineName, bot) {
        await bot.sendMessage(chatId, `⚠️ Medicine "${medicineName}" is found in our system but currently out of stock.`);
    }
    async sendInsufficientStockMessage(chatId, medicineName, available, bot) {
        await bot.sendMessage(chatId, `⚠️ Insufficient stock for "${medicineName}". Only ${available} units available.`);
    }
    async sendPrescriptionProcessedMessage(chatId, medicineName, quantity, cartSize, bot) {
