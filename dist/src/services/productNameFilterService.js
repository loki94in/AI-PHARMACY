...
function similarity(s1, s2, threshold = 0.8) {
        ...
        for (let j = 1; j <= a.length; j++) {
            ...
            for (let i = 1; i <= b.length; i++) {
                if (b.charAt(i - 1) === a.charAt(j - 1)) {
                    ...
                    currRow[i] = Math.min(prevRow[i - 1] + 1, // substitution
...
export class ProductNameFilterService {
    ...
    initialized = false;
    dbPath;
    DEFAULT_THRESHOLD = 0.8; // 80% similarity threshold
    ...
    async filterProductNames(ocrText, options = {}) {
        const startTime = Date.now();
        if (!this.initialized) {
            throw new Error('ProductNameFilterService not initialized. Call initialize() first.');
        }
        // Merge options with defaults
        const { enableInternetFallback = false, internetApiEndpoint, internetApiKey, minConfidenceThreshold = this.DEFAULT_THRESHOLD, fallbackTimeoutMs = this.DEFAULT_TIMEOUT } = options;
        ...
        if (shouldUseFallback) {
            try {
                fallbackUsed = true;
                internetMatches = await this.queryInternetApi(normalizedOcr, internetApiEndpoint || 'https://api.fda.gov/drug/ndc.json', internetApiKey || process.env.OPENFDA_API_KEY, fallbackTimeoutMs, minConfidenceThreshold);
            }
            catch (error) {
    ...
    async queryInternetApi(query, endpoint, apiKey, timeoutMs, minConfidenceThreshold) {
        ...
        try {
            ...
            return uniqueMatches.filter(matchName => similarity(query, matchName.toLowerCase(), minConfidenceThreshold) >= minConfidenceThreshold);
        }
        catch (error) {
            clearTimeout(timeoutId);
