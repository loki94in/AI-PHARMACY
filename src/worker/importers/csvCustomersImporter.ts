import fs from 'fs';
import csvParser from 'csv-parser';

interface Stats { imported: number; skipped: number; errors: number }

export async function runCsvCustomersImport(
  filePath: string,
  db: any,
  onProgress: (processed: number, total: number) => Promise<void>
): Promise<Stats> {
  const rows: Record<string, string>[] = await readCsv(filePath);
  const total = rows.length;
  const stats: Stats = { imported: 0, skipped: 0, errors: 0 };

  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    try {
      const name = (row['name'] || row['customer_name'] || row['patient_name'] || '').trim();
      const phone = (row['phone'] || row['mobile'] || row['contact'] || '').trim();
      if (!name) { stats.skipped++; continue; }

      const existing = await db.get(
        `SELECT id FROM customers WHERE LOWER(TRIM(name)) = LOWER(TRIM(?)) AND COALESCE(phone,'') = ?`,
        [name, phone]
      );
      if (existing) { stats.skipped++; continue; }

      await db.run(
        `INSERT INTO customers (name, phone, address, age, gender, credit_enabled, credit_balance, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          name,
          phone,
          (row['address'] || '').trim(),
          (row['age'] || '').trim(),
          (row['gender'] || '').trim(),
          (row['credit_enabled'] || '0').trim() === '1' ? 1 : 0,
          parseFloat(row['credit_balance'] || '0') || 0,
          (row['notes'] || '').trim(),
        ]
      );
      stats.imported++;
    } catch (err) {
      stats.errors++;
      console.error(`[CsvCustomersImporter] Row ${i + 1} error:`, err);
    }

    if ((i + 1) % 50 === 0 || i === total - 1) {
      await onProgress(i + 1, total);
    }
  }

  return stats;
}

function readCsv(filePath: string): Promise<Record<string, string>[]> {
  return new Promise((resolve, reject) => {
    const rows: Record<string, string>[] = [];
    if (!fs.existsSync(filePath)) return resolve(rows);
    fs.createReadStream(filePath)
      .pipe(csvParser())
      .on('data', (row: any) => rows.push(row))
      .on('end', () => resolve(rows))
      .on('error', reject);
  });
}
