...
export async function flushReturns(db: Database) {
  ...
  for (const r of returnBatch) {
    const result = await db.run(
      `INSERT INTO returns (return_no, original_invoice_id, type, date, total_amount, cgst_value, sgst_value, igst_value, distributor_id, legacy_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [r.return_no, r.original_invoice_id, r.type, r.date, r.total_amount, r.cgst_value, r.sgst_value, r.igst_value, r.distributor_id, r.legacy_id]
    );
